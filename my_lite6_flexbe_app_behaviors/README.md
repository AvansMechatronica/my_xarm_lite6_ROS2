# FlexBE States and Behaviors for my_lite6_flexbe_app
...
This raw repository has several folders and files with the generic name `my_lite6_flexbe_app`.
...
and change the relevant `my_lite6_flexbe_app` text to `my_new_project_name` as needed.
...
## Example States in `my_lite6_flexbe_app_flexbe_states`
...
## Example Behaviors in `my_lite6_flexbe_app_flexbe_behaviors`
...
[`copy_behavior`](my_lite6_flexbe_app_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run my_lite6_flexbe_app_flexbe_behavior copy_behavior` to see the usage guide. 
